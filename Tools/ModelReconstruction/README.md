# BottleCleanCapV25 production assets
...
Unity uses `bottle_no_cap_clean_cap_v25.fbx` under
`Assets/Models/CleanBottleReconstruction/BottleCleanCapV25`. The hierarchy is:
...
`prepare_bottle_clean_cap_v25.py` preserves the approved rigid B+C registration,
adds the clean B-only neck guide, places the mouth seam at the origin, and
keeps B/C transforms at identity. `render_bottle_clean_cap_v25_qa.py`
...
`Assets/Models/CleanBottleReconstruction/BottleCleanCapV25/Textures/bottle_full_clean_v2_albedo.png`.
