# BottleCleanCap v31 A-to-B-to-C contract
...
- C is the approved clean 39 mm x 10 mm `BottleCapC`.

The production Blender source is
`F:\Meshroom_work\bottle_full_clean_v2\split_models\bottle_no_cap_clean_cap_v31.blend`.
...
The B-only reference neck rises 20.00 mm from that cut to the physical mouth.
Its lathed profile follows the supplied real photograph: a narrow lower stem
and tamper-ring transition followed by the upper threaded section. C is
10.12 mm high from its mesh bounds and encloses the upper half of that neck.
Its inner roof is fixed to the raised mouth plane at model Y `0.11764706`, so C
overlaps the upper thread instead of floating over the shoulder. The exact
transforms and dimensions are stored in
`bottle_no_cap_clean_cap_v31_report.json`.
...
algorithm. The v31 native tracker retains the user-aligned world-pose prior,
...
When a pose has at least 18 inliers, at least 70% inlier ratio and low
reprojection error, v31 uses a bounded relaxed spatial-distribution gate. This
accepts the real label's narrow but strong feature band seen in v30 device
evidence while low-consensus poses still use the stricter coverage gate.
